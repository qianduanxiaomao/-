...
export default {
  ...
  methods: {
      ...
      */
      skuInfo.skuSaleAttrValueList = spuSaleAttrList.reduce((pre, attr) => {
        if (attr.saleAttrValueId) {
          const [saleAttrId, saleAttrValueId] = attr.saleAttrValueId.split(':')
          pre.push({ saleAttrValueId, saleAttrId })
        }
        return pre
      }, [])
    ...
    */
    async initUpdateSkuFormData(row, category1, category2) {
      // 取出请求条件数据
      this.spu = row
      // 收集父组件传入的数据到skuInfo
      // category3Id: null, // 3级分类ID
      ...
      const promise1 = this.$API.attr.getAttrList(category1, category2, row.id)
      const promise2 = this.$API.sku.getSpuSaleAttrList(row.id)
      const promise3 = this.$API.sku.getSpuImageList(row.id)
      // 只有当都成功时才更新数据显示  ===>  Promise.all(promises)
      await Promise.all([promise1, promise2, promise3]).then((results) => {
